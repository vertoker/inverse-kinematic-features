...
namespace CustAnim.Splines
{
    ...
    {
        [SerializeField] protected List<SplineJoint> joints = new();
        ...
        public Vector3 Get(float progress)
        {
            ...
            if (progress >= joints.Count - 1)
                return joints[^1].Joint.position;
        ...
        public class SplineJointsEditor : UnityEditor.Editor
        {
            public override void OnInspectorGUI()
            {
                ...
                if (GUILayout.Button("Save Joint"))
                {
                    ...
                    {
                        Debug.LogError("Spline asset save path is not selected");
                        return;
                    }
                    ...
                    AssetDatabase.CreateAsset(data, $"Assets{path.Split("Assets")[^1]}");
                }
                if (GUILayout.Button("Load Joint"))
                {
                    ...
                    if (path == string.Empty)
                    {
                        Debug.LogError("Spline asset is not selected");
                        return;
                    }
                    
                    var data = AssetDatabase.LoadAssetAtPath<SplineData>($"Assets{path.Split("Assets")[^1]}");
